...
> AIとの会話を **Obsidian Vault・Notion・両方**に Atomic Notes としてストック。
...
1. 直前の AI 回答を選んだバックエンドに保存 — **Obsidian Vault**（Markdown）、**Notion**（ページ）、または両方
2. type を自動分類（decision / snippet / learning / reference）
3. セッション単位で振り分け（Obsidian はフォルダ、Notion はサブページ）
4. 同タグの過去 Atom と自動的に相互リンク（Obsidian）
...
LLM エージェントには記憶機能があるが、セッションごとに分断されており、人間が後から見返すのは困難。AI2Stock は会話の **フロー（流れ去るもの）** を **ストック（蓄積されるもの）** に変換する。バックエンドは用途に合わせて選択: ローカル Markdown（Obsidian）、チーム共有 workspace（Notion）、両方並行のいずれも可。

## 機能

- **`/stock` slash command**（Claude Code 用）
- **自然言語操作**: `/stock 30分前のBYOD議論を保存` 等
- **マルチバックエンド**: Obsidian Vault（Markdown + YAML frontmatter）/ Notion（ページツリー、sidebar 表示）
- **コール単位のバックエンド切替**: `/stock --to=obsidian|notion|all`
- **セッション単位の振り分け**: 各 Claude Code セッションごとに Obsidian はフォルダ、Notion はサブページ
- **自動type分類**: decision / snippet / learning / reference
- **同タグ自動リンク**, **プロジェクト別 MOC**（Obsidian）
- **シンボリックリンク安全**, **アトミック書き込み**, **TOCTOU race 対策**（Obsidian）
...
- **npm** （Node.js に同梱）
- **git** （多くの環境に既存）
- **保存先バックエンド**: **Obsidian Vault**（`.md` を入れるフォルダ。Obsidian アプリ本体は任意）/ **Notion** workspace + Internal Integration Token のいずれか（または両方）
...
3. `ai2stock init` を対話的に実行:
   ...
   - デフォルトプロジェクト名を設定（任意）
   - Notion を使うか選択（任意・[Notion アダプタ](#notion-アダプタ任意) 参照）
...
- デフォルトプロジェクト名（任意）
- Notion を有効にするか（Token 環境変数 + Parent Page ID — [Notion アダプタ](#notion-アダプタ任意) 参照）
